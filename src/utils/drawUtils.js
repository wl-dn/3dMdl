 ...
 * @LastEditors: wenlong
 * @LastEditTime: 2021-08-27 14:05:33
 * @FilePath: \cesium-web-vue\src\utils\vue-utils\draw\drawUtils.ts
 */
import {
  Viewer, ScreenSpaceEventHandler, Cartesian3, Cartesian2, ScreenSpaceEventType, Ray, defined, CallbackProperty, Color, Entity, PolygonHierarchy
} from 'cesium'
class DrawPolyline {
  static _viewer = null;
  static _handler = null;
  static _polyline = null;
  _positions = null;
  ...
  getCatesian3FromPX(px) {
    const ray = DrawPolyline._viewer.camera.getPickRay(px);
    const cartesian = DrawPolyline._viewer.scene.globe.pick(ray, DrawPolyline._viewer.scene);
    if (!cartesian) {
...
class DrawPolygon {
  ...
  getCatesian3FromPX(px) {
    const ray = DrawPolygon._viewer.camera.getPickRay(px);
    const cartesian = DrawPolygon._viewer.scene.globe.pick(ray, DrawPolygon._viewer.scene);
    if (!cartesian) {
      throw new Error("未获取到正确坐标");
    }
    return cartesian
